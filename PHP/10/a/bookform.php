<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($title && $price && $page && $authorID && $publicDate) {
            $stmt = $pdo->prepare("INSERT INTO tbbooks(title, author_id, publication_date, price, number_of_pages) VALUES (?,?,?,?,?)");
            $success = $stmt->execute([$title, $authorID, $publicDate, $price, $page]);
            if ($success) {
                $message = 1;
            }
        } 
        else {
            echo '<p class="text-red-500 mb-5">All input are required</p>';
        }
    }
...
<body class=" bg-slate-50">
    <div class=" max-w-md mx-auto my-20 ">
        <?php if($message === 1): ?>
            <p class="text-green-600 mb-5">Book Add Successful !!</p>
        <?php else: ?>
            ...
            <form class=" bg-white p-5 shadow rounded" action="" method="POST">
                ...
                <div class=" mb-3">
                    ...
                    <input type="number" name="price" step="any" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required />
                </div>
                <div class=" mb-3">
                    <label class="block mb-2 text-sm font-medium text-gray-900">Number of pages</label>
                    <input type="number"  name="num_of_page" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required />
                </div>
                <div class="mb-3 flex gap-2">
                    ...
                    <div class=" w-full">
                        ...
                        <input type="date" name="public_date" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required />
                    </div>
                </div>
                <div class=" mb-3">
                    <input class=" text-white cursor-pointer bg-blue-700 hover:bg-blue-800  font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2" type="submit" value="Add book">
                </div>
            </form>
        <?php endif; ?>
