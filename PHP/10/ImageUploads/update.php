...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $target_dir = "images/";
    $target_file = $image ? $target_dir . basename($image) : $person['image'];
    ...
    if (empty($name) || empty($email)) {
        ...
        try {
            ...
            if (!isset($error)) {
                ...
                $sql = "UPDATE persons SET name = ?, email = ?, image = ? WHERE id = ?";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$name,$email,$target_file,$id]);
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="bg-white mt-5 p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 class="text-2xl font-bold mb-4 text-center">Update Person</h2>
        <?php if (isset($error)): ?>
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-4">
            <div class="relative">
                ...
                <input type="text" name="name" id="name" value="<?php echo htmlspecialchars($person['name']); ?>" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required>
            </div>
            <div class="relative">
                ...
                <input type="email" name="email" id="email" value="<?php echo htmlspecialchars($person['email']); ?>" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required>
            </div>
            ...
            <div class="flex items-center justify-center w-full">
                <label for="image" class="flex flex-col items-center justify-center w-full h-50 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50  hover:bg-gray-100">
                    ...
                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                        <svg class="w-8 h-8 mb-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" indexBox="0 0 20 16">
            ...
            <img src="<?php echo htmlspecialchars($person['image']); ?>" alt="" class="mt-2 w-24 h-24 object-cover rounded-md">
            <button type="submit" class="w-full bg-indigo-600 text-white py-2 px-4 rounded-md hover:bg-indigo-700">Update</button>
        </form>
        <a href="index.php" class="block text-center mt-4 text-indigo-600 hover:text-indigo-800">Back to List</a>
    </div>
</body>
</html>