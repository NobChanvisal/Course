...
@section('content')
    <div class="pt-3">
        <div>
            <p class=" text-base text-slate-500">To add a new product, please fill in the form below:</p>
        </div>
            {{-- main --}}
            @if ($errors->any())
                        ...
                        <div class=" border border-slate-400 rounded-xl p-3 mt-4">
                            <label class=" text-center " for="image">
                                <img id="previewImage" 
                                    ...
                                    src="https://i.pinimg.com/1200x/5b/7f/64/5b7f6416ba620c37a617f17a54fd450f.jpg">
                    ...
                    <div class=" flex-1 border rounded-2xl p-4">
                        <div>
                            ...
                            <div class=" mt-3">
                                ...
                                <textarea name="description" id="description" rows="5" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">{{ old('description') }}</textarea>
                            ...
                            <div class="mt-3">
                                ...
                                <div class=" flex gap-4 mt-1">
                                    <div class="flex items-center px-4 border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md">
                                        <input id="no_dis" type="radio" name="price_type" checked  value="none">
                                        <label for="no_dis" class="w-full py-2 select-none ms-2 text-sm font-medium text-heading">No discount</label>
                                    </div>
                                    <div class="flex items-center px-4 border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md">
                                        <input id="percent_dis" type="radio" name="price_type" value="percent">
                                        <label for="percent_dis" class="w-full py-2 select-none ms-2 text-sm font-medium text-heading">Percent Discount</label>
                                    </div>
                                    <div class="flex items-center px-4 border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md">
                                        <input id="fixed_dis" type="radio" name="price_type" value="fixed">
                                        <label for="fixex_price" class="w-full py-2 select-none ms-2 text-sm font-medium text-heading">Fixed Price</label>
                                    </div>
                                </div>
                            </div>
                            <div id="percentWrapper" class=" mt-3 sr-only">
                                <x-input-label for="percent_price" :value="__('Discount percentage(%)')" />
                                <x-text-input id="percent_price" name="discount_percentage" type="number" class="mt-1 block w-full" :value="old('discount_percentage',0)"  />
                                <x-input-error class="mt-2" :messages="$errors->get('discount_percentage')" />
                            </div>
                            <div id="fixedWrapper" class=" mt-3 sr-only">
                                <x-input-label for="fixed_price" :value="__('Fixed Price($)')" />
                                <x-text-input id="fixed_price" name="discounted_price" max="99" type="number" class="mt-1 block w-full" :value="old('discounted_price',0)" />
                                <x-input-error class="mt-2" :messages="$errors->get('discounted_price')" />
                            </div>
                            <div class=" mt-6 max-w-52 ml-auto flex justify-end">
                                <button type="submit" class=" bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-600">Create Product</button>    
                            </div>
                        </div>
                    </div>
