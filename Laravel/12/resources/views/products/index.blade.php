            ...
            <tbody>
                @foreach ($products as $pro)
                    <tr>
                        ...
                        <td class=" border px-4 py-2">{{$pro->categories ? $pro->categories->name : 'No Category'}}</td>
                        ...
                        <td class=" border px-4 py-2 text-center ">
                            <span class="flex items-center justify-center px-1.5 py-0.5 text-xs font-medium capitalize rounded {{ $pro->status == 'available' ? 'text-green-500 bg-green-200' : 'text-red-500 bg-red-200' }} " >
                                <span class="h-1.5 w-1.5 {{ $pro->status == 'available' ? 'bg-green-500' : 'bg-red-500' }} rounded-full me-1"></span>
                                {{$pro->status}}
                            </span>
                        </td>
                        <td class=" border px-4 py-2 text-nowrap">{{$pro->updated_at->format('d-m-Y')}}</td>
                        <td class=" border px-4 py-2">
