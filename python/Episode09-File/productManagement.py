...
def save_to_file(product_list, filename="products.txt"):
    try:
...
def load_from_file(filename="products.txt"):
    product_list = []
    try:
...
while True:
    ...
    if opt == 1:  # Insert products
        try:
            ...
            save_to_file(product_list)  # Save after adding
        except ValueError:
        ...
        if not product_list:
            ...
            for product in product_list:
                if product.code == code_to_remove:
                    ...
                    found = True
                    save_to_file(product_list)  # Save after deleting
                    break
            if not found:
        ...
        if not product_list:
            ...
            for product in product_list:
                if product.code == code_to_update:
                    ...
                    found = True
                    save_to_file(product_list)  # Save after updating
                    break
